...
class AnalystAgent:
    ...
    async def analyze(self, question: str, history: list[dict] | None = None, hints: list[str] | None = None) -> tuple[AnalystResponse, dict]:
        messages = [{"role": "system", "content": ANALYST_PROMPT}]

        if history:
            messages.extend(history[-config.ANALYST_HISTORY_LIMIT:])
        ...
        if parsed is not None:
            try:
                ...
                logger.error("Failed to build AnalystResponse: %s\nParsed: %s", e, parsed)
            ...
            analyst_response = AnalystResponse(
                queries=[SearchQuery(text=question)],
                analysis="Не удалось разобрать ответ аналитика.",
                confidence=0.3,
            )
    ...
    async def summarize(self, results: list[SearchResult], question: str) -> tuple[SummarizedContext, dict]:
        if not results:
...
Важно: только валидный JSON, без markdown."""
        ...
        if parsed is not None:
            try:
                ...
                logger.error("Failed to build SummarizedContext: %s\nParsed: %s", e, parsed)
                ctx = SummarizedContext(
                    summary=content[:500],
            ...
            logger.error("Failed to parse Summarize response\nContent: %s...", content[:200])
            ctx = SummarizedContext(
                summary=content[:500],
                citations=[r.content[:200] for r in results[:3]],
                files_involved=list(files),
