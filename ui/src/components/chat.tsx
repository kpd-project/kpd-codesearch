    ...
    try {
        ...
        return parsed
            .map((item) => {
                ...
                if (m.role === 'assistant' && m.answerFooter != null && !isAnswerFooterMeta(m.answerFooter)) {
...
function RagModeToolbar({
    ...
    return (
        <div
            className="inline-flex h-8 shrink-0 overflow-hidden rounded-md border border-border bg-muted/50"
        ...
        >
            <Button
                ...
                className="h-8 rounded-none border-0 px-2.5 text-xs shadow-none"
                disabled={disabled}
            ...
            <Button
                ...
                className="h-8 rounded-none border-0 border-l border-border px-2.5 text-xs shadow-none"
                disabled={disabled}
...
/** Список сообщений отдельно от поля ввода: при наборе текста не ре-рендерится markdown по всей истории. */
const ChatMessagesView = memo(function ChatMessagesView({
    ...
    onCopyMessage: (text: string, messageIndex: number) => void;
    onOpenSessionLog: (logId: string) => void;
            ...
            <div className="mx-auto w-full max-w-[120ch] px-4 py-4 space-y-4">
                {messages.length === 0 && (
                    <div className="text-center text-muted-foreground mt-20">
                        ...
                        <p className="text-sm mt-2">Например: «Как работает аутентификация?»</p>
                    </div>
                )}
                        ...
                        >
                            ...
                            >
                                ...
                                <Button
                                    ...
                                    onClick={() => void onCopyMessage(msg.content, i)}
                                >
                                    ...
                                    {msg.answerFooter.log_id ? (
                                        <p>
                                            <button
                                                ...
                                                onClick={() => onOpenSessionLog(msg.answerFooter!.log_id)}
                                            >
...
/** Локальный `input`: набор текста не триггерит ре-рендер родителя и списка сообщений. */
            ...
            <div className="mx-auto w-full max-w-[120ch] px-4 py-4">
                <form onSubmit={handleSubmit} className="flex flex-wrap gap-2 items-start">
                    <Textarea
                        ...
                        rows={3}
                        className="field-sizing-fixed min-h-20 max-h-48 flex-1 min-w-48 resize-y overflow-y-auto"
                    />
...
export function Chat({ className, ragMode, onRagModeChange, ragModeDisabled }: ChatProps) {
    ...
    const copyMessageText = useCallback(async (text: string, messageIndex: number) => {
        const t = text.trim();
        if (!t) return;
        try {
    ...
    const openSessionLog = useCallback((logId: string) => {
        setSessionLogModalId(logId);
    }, []);
    ...
    return (
        <div className={cn('flex flex-col h-full', className)}>
            ...
            <ChatComposer
                ...
                ragMode={ragMode}
                onRagModeChange={onRagModeChange}
                ragModeDisabled={ragModeDisabled}
