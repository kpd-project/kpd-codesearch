...
export function AddRepoDialog({ open, onOpenChange, onAdd, onOpenExisting, basePath = '' }: AddRepoDialogProps) {
    ...
    const loadCandidates = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(typeof detail === 'string' ? detail : 'Не удалось загрузить список папок');
            }
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent className="sm:max-w-2xl">
                ...
                <div className="space-y-4 mt-2">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <TooltipProvider>
                                <Tooltip>
                                    ...
                                    <TooltipContent>
                                        Список папок ниже — это подкаталоги этого каталога. Чтобы изменить базовый путь,
                                        задайте переменную <span className="font-mono">REPOS_BASE_PATH</span> в{' '}
                                        <span className="font-mono">.env</span> и перезапустите backend.
                                    </TooltipContent>
                                </Tooltip>
                            </TooltipProvider>
                        </div>
                        <p className="text-foreground font-mono text-sm break-all">{basePath || '—'}</p>
                    </div>
                    ...
                    <ScrollArea
                        keepMountedScrollbar
                        className="h-[min(50vh,420px)] min-h-0 rounded-lg border border-border"
                    >
                        <div className="p-1">
                            {loading && <p className="text-muted-foreground text-sm p-3">Загрузка…</p>}
                            {!loading && !loadError && filtered.length === 0 && (
                                <p className="text-muted-foreground text-sm p-3">Нет подходящих папок</p>
                            )}
                            {!loading &&
                                filtered.map((c) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {c.already_added && (
                                                <Badge
                                                    variant="outline"
                                                    className="shrink-0 border-emerald-200/80 bg-emerald-100 text-emerald-950 dark:border-emerald-800/80 dark:bg-emerald-950/40 dark:text-emerald-100"
                                                >
                                                    Уже добавлено
                                                </Badge>
                                            )}
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                        <Button
                            type="button"
                            variant="secondary"
                            disabled={!selected?.already_added || !selected.collection_name || submitting}
                            onClick={handleOpen}
                        ...
                        <Button
                            type="button"
                            disabled={!selected || selected.already_added || submitting || loading}
                            onClick={handleAdd}
                            className="w-full sm:w-auto"
                        >
